...
function JudgementDetailPage({ client, id, onBack, onRetry, notify }: { client: HanaiClient; id: string; onBack: () => void; onRetry: (stock: SearchResult, masterId: string) => void; notify: Notify }) {
  ...
  return <Page>
    ...
    {!ready ? <article className={`${styles['card']} ${styles['liveProcess']}`}>
      ...
      {sessionId === null ? <Empty title="研判会话正在准备" detail="DSH Session 建立后将在这里显示实时执行过程。" /> : <ChatPanel key={`${id}:${sessionId}:live`} clientContext={client.ctx} sessionId={sessionId} title="实时研判过程" readOnlyReason="报告生成期间仅查看执行过程；报告封存后才可继续对话。" />}
      ...
      {view === 'process' && <article className={`${styles['card']} ${styles['archivedProcess']}`}>{sessionId === null ? <Empty title="研判过程不可用" detail="这份归档未关联 DSH Session。" /> : <ChatPanel key={`${id}:${sessionId}:process`} clientContext={client.ctx} sessionId={sessionId} title="研判过程" readOnlyReason="已归档的研判过程为只读记录。" />}</article>}
      {view === 'chat' && <article className={`${styles['card']} ${styles['continuedChat']}`}>{sessionId === null ? <Empty title="对话不可用" detail="这份报告未关联 DSH Session。" /> : <ChatPanel key={`${id}:${sessionId}:chat`} clientContext={client.ctx} sessionId={sessionId} title={`继续与${judgement.masterName}对话`} />}</article>}
    </div>}
  </Page>
}
